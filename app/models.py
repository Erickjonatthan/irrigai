...
# Definindo os valores de Kc por cultura e estágio de desenvolvimento
kc_values = {
...
def PlotGrafico(data, name='', _NomeLocal='', _graficos=''):
    ...
    # Corrigir o caminho do arquivo usando os.path.join
...
def get_location_name(latitude, longitude):
    try:
        ...
        headers = {
            "User-Agent": "PaleBlueDot-DuneDivers/1.0 (seu-email@example.com)"  # Substitua pelo seu e-mail real
        }
        
        # Fazer uma requisição GET com os cabeçalhos
        ...
        # Verificar se o campo 'address' está presente
...
def criar_diretorios(inDir, latitude, longitude, area):
    """
    Cria os diretórios necessários para armazenar dados e gráficos.

    Args:
        inDir (str): Diretório base para armazenamento.
        latitude (float): Latitude da região.
        longitude (float): Longitude da região.
        area (float): Área da região.

    Returns:
        tuple: Caminhos para os diretórios de dados locais e gráficos.
    """
    ...
    print(f"Dados serão armazenados em: {_localdataName}")
    print(f"Gráficos serão armazenados em: {_graficos}")
...
def criar_mapa(latitude, longitude, _quadrado, _graficos):
    ...
    Args:
        latitude (float): Latitude da região.
        longitude (float): Longitude da região.
        _quadrado (list): Coordenadas do polígono.
        _graficos (str): Caminho para o diretório onde o mapa será salvo.

    Returns:
        str: Caminho do arquivo HTML do mapa gerado.
    """
    data_Json = {
...
def gerar_grafico_precipitacao(_dados, _ano_inicial, _ano_final, _NomeLocal, _graficos):
    ...
    Args:
        _dados (pd.DataFrame): Dados contendo os anos e os valores de precipitação.
        _ano_inicial (int): Ano inicial do intervalo.
        _ano_final (int): Ano final do intervalo.
        _NomeLocal (str): Nome da região.
        _graficos (str): Caminho para o diretório onde o gráfico será salvo.

    Returns:
        str: Caminho do arquivo do gráfico gerado.
    """
...
def recomendar_irrigacao(cultura, estagio, dados_ET, dados_PET):
    """
    Fornece recomendações de irrigação com base na cultura, estágio de desenvolvimento,
    evapotranspiração real (ET) e evapotranspiração potencial (PET).

    Parâmetros:
    - cultura (str): Nome da cultura (exemplo: 'milho', 'feijao', 'tomate', 'cana-de-acucar')
    - estagio (str): Estágio de desenvolvimento ('inicial', 'medio' ou 'final')
    - dados_ET (pd.Series): Série com valores diários de ET
    - dados_PET (pd.Series): Série com valores diários de PET

    Retorna:
    - pd.DataFrame: DataFrame com recomendações diárias de irrigação
    """
